...
sc_alldata <- as.data.frame(sc_alldata)
sc_alldata_mems <- sc_alldata %>% filter(sc_alldata$`Member Type` %in% c("M","MN"))
...
#Below converts "No" into reasoning values
sc_alldata_mems <- sc_alldata_mems %>%
  mutate(reasoning = if_else(type == "No", substr(reasoning, 2, nchar(reasoning)), reasoning))
sc_alldata_mems <- sc_alldata_mems %>% mutate(type = if_else(type == "No",reasoning, type))
...
```{r}
#Sales breakdown NEED TO ASK QUESTIONS ON THIS!
sc_mems_sales <- sc_alldata_mems %>% filter(type == "Sales")
plot_mem_sales <- ggplot(sc_mems_sales, aes(x = sc_mems_sales$reasoning))+ geom_bar(stat = "count", fill = "purple")+ geom_text(stat = "count", aes(label = after_stat(count)), vjust = -0.25) +labs(title = "Category of Sales SC", x = "Reasoning" , y = "# of Scorecards")+
  theme(axis.text.x = element_text(angle = 45, hjust = 1))  
plot_mem_sales
